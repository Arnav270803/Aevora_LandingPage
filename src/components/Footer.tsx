import { useEffect, useState } from 'react';

const Footer = () => {
  const [isMobile, setIsMobile] = useState(false);

  useEffect(() => {
    const check = () => setIsMobile(window.innerWidth < 640);
    check();
    window.addEventListener('resize', check);
    return () => window.removeEventListener('resize', check);
  }, []);

  return (
    <footer
      style={{
        borderTop: '1px solid rgba(232, 98, 42, 0.12)',
        background: 'rgba(255, 252, 249, 0.80)',
        backdropFilter: 'blur(14px)',
        WebkitBackdropFilter: 'blur(14px)',
        padding: isMobile ? '20px 24px' : '18px 48px',
      }}
    >
      <div
        style={{
          display: 'flex',
          flexDirection: isMobile ? 'column' : 'row',
          alignItems: 'center',
          justifyContent: 'space-between',
          gap: isMobile ? 16 : 0,
        }}
      >
        {/* Left — logo + copyright */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <svg width="18" height="18" viewBox="0 0 34 34" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M6 5 C4 3, 3 6, 4.5 9 C5.5 11, 8 12, 10 12.5" stroke="#e8622a" strokeWidth="2.2" strokeLinecap="round" fill="none"/>
            <path d="M28 5 C30 3, 31 6, 29.5 9 C28.5 11, 26 12, 24 12.5" stroke="#e8622a" strokeWidth="2.2" strokeLinecap="round" fill="none"/>
            <ellipse cx="17" cy="21" rx="9" ry="8" fill="#e8622a"/>
            <ellipse cx="17" cy="25.5" rx="5.5" ry="3.5" fill="#c4501f"/>
            <circle cx="15" cy="25.8" r="1.2" fill="#0d0d0d" opacity="0.6"/>
            <circle cx="19" cy="25.8" r="1.2" fill="#0d0d0d" opacity="0.6"/>
            <circle cx="13" cy="19" r="1.6" fill="#0d0d0d"/>
            <circle cx="21" cy="19" r="1.6" fill="#0d0d0d"/>
            <circle cx="13.6" cy="18.4" r="0.5" fill="white"/>
            <circle cx="21.6" cy="18.4" r="0.5" fill="white"/>
          </svg>
          <span
            style={{
              fontFamily: 'monospace',
              fontSize: 13,
              fontWeight: 700,
              color: '#374151',
              letterSpacing: '-0.01em',
            }}
          >
            Ae<span style={{ color: '#e8622a' }}>vora</span>
          </span>
          <span style={{ color: '#e5e7eb', fontFamily: 'monospace', fontSize: 13 }}>·</span>
          <span
            style={{
              fontFamily: 'monospace',
              fontSize: 11,
              fontWeight: 500,
              color: '#9ca3af',
            }}
          >
            v0.1 · early access
          </span>
        </div>

        {/* Center — tagline, hidden on mobile */}
        {!isMobile && (
          <span
            style={{
              fontFamily: 'monospace',
              fontSize: 10,
              fontWeight: 600,
              color: '#d1d5db',
              letterSpacing: '0.14em',
              textTransform: 'uppercase',
              position: 'absolute',
              left: '50%',
              transform: 'translateX(-50%)',
            }}
          >
            ship video ads in under 2 min
          </span>
        )}

        {/* Right — built by */}
        <a
          href="https://www.hayarnav.xyz/"
          target="_blank"
          rel="noopener noreferrer"
          style={{
            display: 'inline-flex',
            alignItems: 'center',
            gap: 5,
            textDecoration: 'none',
            fontFamily: 'monospace',
            fontSize: 11,
            fontWeight: 600,
            color: '#9ca3af',
            letterSpacing: '0.02em',
            transition: 'color 0.2s',
          }}
          onMouseEnter={(e) => {
            (e.currentTarget as HTMLAnchorElement).style.color = '#e8622a';
          }}
          onMouseLeave={(e) => {
            (e.currentTarget as HTMLAnchorElement).style.color = '#9ca3af';
          }}
        >
          Built by founder
          <svg
            width="9"
            height="9"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            strokeWidth="2.5"
            strokeLinecap="round"
            strokeLinejoin="round"
            style={{ opacity: 0.6 }}
          >
            <path d="M7 17L17 7M17 7H7M17 7v10"/>
          </svg>
        </a>
      </div>
    </footer>
  );
};

export default Footer;